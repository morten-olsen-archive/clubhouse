...
describe('members', () => {
  ...
  it('should be able to encrypt for multible users', async () => {
    ...
    const decrypted1 = await members.decrypt(
      encrypted,
      [user3.pub, user2.pub, user1.pub],
      user2.priv,
    );
    expect(decrypted1.message).to.be.eq('foo');
    expect(decrypted1.signatures[0].valid).to.be.eq(true);
    const decrypted2 = await members.decrypt(
      encrypted,
      [user3.pub, user2.pub, user1.pub],
      user3.priv,
    );
    ...
    try {
      ...
      throw new Error('user should not be allowed');
    } catch (err) {
      expect(err.toString()).to.not.equal('user should not be allowed');
    }
  });
